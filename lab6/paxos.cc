         ...
         std::string &v)
{
  ...
  for (int i = 0; i < nodes.size(); i++) {
    handle h(nodes[i]);
    /* The caller probably should not hold a mutex when calling safebind. */
        ...
        std::vector<std::string> nodes, std::string v)
{
  ...
  for (int i = 0; i < nodes.size(); i++) {
    ...
    pthread_mutex_lock(&pxs_mutex);
    if (cl) {
      ret = cl->call(paxos_protocol::acceptreq, me, arg, res, rpcc::to(1000));
      if (ret == paxos_protocol::OK && res) {
        accepts.push_back(nodes[i]);
      }
    }
  }
	      ...
	      std::string v)
{
  ...
  for (int i = 0; i < accepts.size(); i++) {
    ...
    pthread_mutex_lock(&pxs_mutex);
    if (cl) {
      ret = cl->call(paxos_protocol::decidereq, me, arg, res, rpcc::to(1000));
    }
  }
}
