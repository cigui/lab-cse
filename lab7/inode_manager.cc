...
/*
 * Count the 0 bits in a char which is supposed to be from 0x01 to 0x0f
 */
int 
count0(char c) {
  if (c == 0x0f) return 0;
  else if (c == 0x00) return 4;
  else if (c == 0x0e || c == 0x0d || c == 0x0b || c == 0x07) return 1;
  else if (c == 0x01 || c == 0x02 || c == 0x04 || c == 0x08) return 3;
  else return 2;
}

/*
 * Get a byte of data from an array of char whose size is 4.
 * For example, the content of the array (in bits) can be :
 * 1111 0000 0000 0000 0000 1111 1111 0000
 * The example above is (in short) 10000110, which is the reverse of char
 * 'a'(0x61, 01100001), so the function returns 'a' for this example.
 */
char 
get_byte(char *buf) {
  char pos[8];
  char result = 0x00, low, high;
  for (int i = 0; i < 8; i++)  pos[i] = 0x00;
  for (int i = 0; i < 4; i++) {
    low = buf[i] & 0x0f;
    high = (buf[i] >> 4) & 0x0f;
    if (count0(high) < 2) pos[i * 2] = 0x01;
    if (count0(low) < 2) pos[i * 2 + 1] = 0x01;
  }
  for (int i = 0; i < 8; i++) {
    result = result | (pos[i] << i);
  }
  return result;
}
...
block_manager::read_block(uint32_t id, char *buf)
{
  ...
  // read 4 blocks
  d->read_block(id * 4, content);
  d->read_block(id * 4 + 1, content + BLOCK_SIZE);
  d->read_block(id * 4 + 2, content + BLOCK_SIZE * 2);
  d->read_block(id * 4 + 3, content + BLOCK_SIZE * 3);
  for (int i = 0; i < BLOCK_SIZE; i++) {
    // calculate each byte
    // 4 byte block content -> 1 byte data
    buf[i] = get_byte(content + i * 4);
  }
}
...
{
  char content[BLOCK_SIZE * 4], tmp;
  for (int i = 0; i < BLOCK_SIZE; i++) {
    tmp = buf[i];
    for (int j = 0; j < 4; j++) {
      // determine content [i*4+j]
      char twobit = (tmp >> (j * 2)) & 0x03;
      char det;
      switch (twobit) {
        case (0x00):
          det = 0; 
  ...
  // write 4 blocks
  d->write_block(id * 4, content);
  d->write_block(id * 4 + 1, content + BLOCK_SIZE);
  d->write_block(id * 4 + 2, content + BLOCK_SIZE * 2);
  d->write_block(id * 4 + 3, content + BLOCK_SIZE * 3);
}
  ...
  if (num <= NDIRECT) {
    for (unsigned int i = 0; i < num; i++) {
      if (rest >= BLOCK_SIZE) {
        ...
        memcpy(block, buf + i * BLOCK_SIZE, rest);
